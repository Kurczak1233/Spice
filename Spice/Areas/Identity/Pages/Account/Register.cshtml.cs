...
namespace Spice.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        {
            //Nazwa naszego radiobutton to: rdUserRole więc pobieramy tą wartość
            ...
            {
                var user = new ApplicationUser  //Zmieniłem na APPLICATIONUSER bo to jest profil który sam rozszerzyłem
                {
                ...
                var result = await _userManager.CreateAsync(user, Input.Password);  //Tworzymy użytkownika
                if (result.Succeeded)
                {
                    //Tworzmy rolę jeśli nie istnieją - wklejamy statyczne role do SD(Utilities) na statyczne rzeczy
                    ...
                    return RedirectToAction("Index", "User", new { area = "Admin" }); //Przekierowanie strony do controlera w adminie
                    //}
                }
